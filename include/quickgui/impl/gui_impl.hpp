...
auto Gui::Start() -> void {
  ...
  DLOG(INFO) << "Gui " << thread_.get_id() << " started by thread "
             << std::this_thread::get_id() << ".";
}

auto Gui::StartInThread() -> Thread {
  return std::thread([]() {
...
auto Gui::LoadProjection() -> void {
  ...
  gluPerspective(kViewFov_, float(display_w) / display_h, kMinClipDistance_,
                 kMaxClipDistance_);
  ...
  gluLookAt(camera_pos_.x(), camera_pos_.y(), camera_pos_.z(),
            (camera_pos_ + camera_front).x(), (camera_pos_ + camera_front).y(),
            (camera_pos_ + camera_front).z(), camera_up.x(), camera_up.y(),
            camera_up.z());
}
...
auto Gui::HandleInput() -> void {
  ...
  if (ImGui::IsKeyPressed(GLFW_KEY_W)) {
    TranslateCamera(camera_z * disp);
  }
  if (ImGui::IsKeyPressed(GLFW_KEY_S)) {
    TranslateCamera(-camera_z * disp);
  }
  if (ImGui::IsKeyPressed(GLFW_KEY_A)) {
    TranslateCamera(camera_x * disp);
  }
  if (ImGui::IsKeyPressed(GLFW_KEY_D)) {
    TranslateCamera(-camera_x * disp);
  }
  if (ImGui::IsKeyPressed(GLFW_KEY_Q)) {
    TranslateCamera(Vector3::UnitY() * disp);
  }
  ...
  if (!ImGui::IsWindowFocused(ImGuiFocusedFlags_AnyWindow) &&
    ...
    auto ang_disp_scale = kNavigationRotationVelocity_ * frame_dt;

    camera_quat_ =
        root_quat *
        Eigen::AngleAxisd(-delta.x * ang_disp_scale, Vector3::UnitY()) *
        Eigen::AngleAxisd(delta.y * ang_disp_scale, Vector3::UnitX());
...
auto Gui::TranslateCamera(const Vector3& dt) -> void {
  camera_pos_ = camera_pos_ + dt;
};
